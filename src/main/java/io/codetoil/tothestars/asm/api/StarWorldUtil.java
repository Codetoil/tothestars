...
public class StarWorldUtil {
    ...
    public static boolean registerStar(int id, boolean initialiseDimensionAtServerInit, int defaultID) {
        ...
        if (initialiseDimensionAtServerInit) {
            ...
            } else if (DimensionManager.getProviderType(id).getId() == id && GalacticraftRegistry
                    .isDimensionTypeIDRegistered(id)) {
    ...
    public static void registerStarClient(Integer dimID, int providerIndex) {
        ...
        if (typeID == 0) {
            TTSCoremod.logger.error("Server dimension {} has no match on client due to earlier registration" +
                    " problem.", dimID);
        } else if (dimID == 0) {
            TTSCoremod.logger.error("Client dimension {} has no match on server - probably a server" +
                    " dimension ID conflict problem.", providerIndex);
            ...
            TTSCoremod.logger.error("Dimension already registered to another mod: unable to register planet" +
                    " dimension {}", dimID);
        }
    }
}
