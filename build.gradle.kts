...
@Suppress("PropertyName")
val galacticraft_legacy_file_id: String by project
...
minecraft {
    ...
    args += "-Dfml.coreMods.load=" + if (use_mixins.toBoolean()) "zone.rong.mixinbooter.MixinBooterPlugin," else "" +
            "micdoodle8.mods.miccore.MicdoodlePlugin" + if (use_coremod.toBoolean()) ",$coremod_plugin_class_name" else ""
    args += "-Dfml.debugAccessTransformer=true"
    args += "-Dforge.logging.console.level=debug"
    args += "-Dforge.logging.markers=SCAN,REGISTRIES,REGISTRYDUMP"
...
dependencies {
    ...
    implementation(rfg.deobf("curse.maven:galacticraft-legacy-564236:${galacticraft_legacy_file_id}"))

    // compile against the JEI API but do not include it at runtime
    compileOnly(rfg.deobf("mezz.jei:jei_1.12.2:${jei_version}:api"))
    // at runtime, use the full JEI jar
    runtimeOnly(rfg.deobf("mezz.jei:jei_1.12.2:${jei_version}"))
}
    ...
    manifest {
        ...
        if (use_coremod.toBoolean()) {
            ...
            if (include_mod.toBoolean()) {
                ...
                attributeMap["ForceLoadAsMod"] = true.toString()//(project.gradle.startParameter.taskNames[0] == "build").toString()
            }
        }
        if (use_access_transformer.toBoolean()) {
